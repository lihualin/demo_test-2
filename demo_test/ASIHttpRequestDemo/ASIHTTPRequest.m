	...
	if ([self shouldAttemptPersistentConnection]) {
		...
		if ([self connectionInfo]) {
			...
			// Check if we should have expired this connection
				...
				ASI_DEBUG_LOG(@"[CONNECTION] Not re-using connection #%i because it has expired",[[[self connectionInfo] objectForKey:@"id"] integerValue]);
				#endif
		...
		if (![self connectionInfo]) {
			...
			[[self connectionInfo] setObject:[NSNumber numberWithInt:[[[self url] port] integerValue]] forKey:@"port"];
		...
		#if DEBUG_PERSISTENT_CONNECTIONS
		ASI_DEBUG_LOG(@"[CONNECTION] Request #%@ will use connection #%i",[self requestID],[[[self connectionInfo] objectForKey:@"id"] integerValue]);
		#endif
...
- (void)saveCredentialsToKeychain:(NSDictionary *)newCredentials
{
	...
	if (authenticationCredentials) {
		[ASIHTTPRequest saveCredentials:authenticationCredentials forHost:[[self url] host] port:[[[self url] port] integerValue] protocol:[[self url] scheme] realm:[self authenticationRealm]];
	}	
}
...
- (NSMutableDictionary *)findCredentials
{
	...
	if ((!user || !pass) && useKeychainPersistence) {
		NSURLCredential *authenticationCredentials = [ASIHTTPRequest savedCredentialsForHost:[[self url] host] port:[[[self url] port] integerValue] protocol:[[self url] scheme] realm:[self authenticationRealm]];
		if (authenticationCredentials) {
			user = [authenticationCredentials user];
			pass = [authenticationCredentials password];
			#if DEBUG_HTTP_AUTHENTICATION
...
- (void)useDataFromCache
{
	...
	if (headers && dataPath) {
		...
		[self setResponseStatusCode:[[headers objectForKey:@"X-ASIHTTPRequest-Response-Status-Code"] integerValue]];
...
- (BOOL)configureProxies
{
	...
	if (![self isPACFileRequest] && (![self proxyHost] && ![self proxyPort])) {
		...
		if ([proxies count] > 0) {
			...
			[self setProxyPort:[[settings objectForKey:(NSString *)kCFProxyPortNumberKey] integerValue]];
...
#if DEBUG_PERSISTENT_CONNECTIONS
			ASI_DEBUG_LOG(@"[CONNECTION] Closing connection #%i because it has expired",[[existingConnection objectForKey:@"id"] integerValue]);
#endif
...
- (NSDictionary *)findSessionProxyAuthenticationCredentials
{
	...
	for (NSDictionary *theCredentials in sessionCredentialsList) {
		if ([[theCredentials objectForKey:@"Host"] isEqualToString:[self proxyHost]] && [[theCredentials objectForKey:@"Port"] integerValue] == [self proxyPort]) {
			[sessionCredentialsLock unlock];
...
+ (NSString *)defaultUserAgentString
{
	@synchronized (self) {
		...
		if (!defaultUserAgent) {
			...
			[self setDefaultUserAgentString:[NSString stringWithFormat:@"%@ (%@ %@; %@)", appVersion, OSName, OSVersion, locale]];
		}
